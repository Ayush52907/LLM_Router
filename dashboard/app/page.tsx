...
import { X, CheckCircle2, Clock, Leaf, FileText, ChevronDown, ChevronUp, Copy, Check, Sparkles, Cpu, Play, LogOut, User } from 'lucide-react';
...
export default function MissionControlDashboard() {
  ...
  const handleRunSelectedModel = async (overrideModelId?: string) => {
    ...
    try {
      const res = await authFetch(`${API_BASE}/api/run-model`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!res || !res.ok) {
        ...
        throw new Error(errData?.error || `Failed to run model: ${res ? res.statusText : 'Authentication error'}`);
      }
